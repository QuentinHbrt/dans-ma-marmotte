  ...
  Mutation: {
      ...
      try {
        ...
        if (count > 0) {
          throw new UserInputError(
            `La pièce ${roomToRemove.name} est utilisée par ${count} rangement(s)`
          );
        }
    ...
    addStorage: async (_, { name, roomId }) => {
      try {
        ...
        if (!roomToAssociate) {
          throw new UserInputError(
            `la pièce demandée pour ce rangement n'existe pas`
          );
        }
        const results = await client.query(
    ...
    removeStorage: async (_, { id }) => {
      ...
      if (count > 0) {
        throw new UserInputError(
          `Le rangement ${storageToRemove.name} est utilisé par ${count} pièce(s)`
        );
      }
    ...
    addProduct: async (_, { name, category, storageId }) => {
      try {
        ...
        if (!storageToAssociate) {
          throw new UserInputError(
            `le rangement demandé pour ce produit n'existe pas`
          );
        }
        const results = await client.query(
